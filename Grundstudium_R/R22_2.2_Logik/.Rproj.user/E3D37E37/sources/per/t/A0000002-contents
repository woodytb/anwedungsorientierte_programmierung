...
alleEinige <- function(zahlen, wert) {
  c(all(zahlen > wert),any(zahlen > wert))
}
...
anzahlAnteil <- function(zahlen, wert) {
  c(sum(zahlen > wert),mean(zahlen > wert))
}
...
zweitesMaximum <- function(x) {
  sort(x, decreasing = TRUE)[2]  # Direkt das zweitgrößte Element
}
