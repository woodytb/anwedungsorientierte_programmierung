...
datum_mit_minus <- function(datum) {
  datum_bereinigt <- gsub("-", "", datum)
  paste0(
    substr(datum_bereinigt, 1, 4), "-",
    substr(datum_bereinigt, 5, 6), "-",
    substr(datum_bereinigt, 7, 8))
}
...
anzahlWorte <- lengths(strsplit(Rtext, "\\s+"))
...
textSätze <- strsplit(Rtext, "(?<=[.!?])\\s+", perl = TRUE)[[1]]


# B2 ----
sentence <- "Dies soll eine Schlagzeile in einer Zeitung werden."
# Schreibe die Anfangsbuchstaben aller Wörter groß.
großgeschrieben <- gsub("(^|\\s)([a-z])", "\\1\\U\\2", sentence, perl = TRUE)
print(großgeschrieben)
