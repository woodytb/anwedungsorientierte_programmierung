...
mehraugen <- factor(c(as.character(mehraugen), "nussbraun"))
...
personenMitFarbe <- function(namen, farben) {
  haeufigkeiten <- table(farben)
  haeufigste_farbe <- names(haeufigkeiten)[which.max(haeufigkeiten)]
...
mmmmEinkommen <- tapply(einkommen, augenfarbe, function(x) {
...
# tb_start
mmmmEinkommenKurz <- tapply(einkommen,augenfarbe,function(x)c(min(x),median(x),mean(x),max(x)))
# tb_ende
